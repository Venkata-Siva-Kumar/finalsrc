...
export default function BannerTab() {
  ...
  const [loadingId, setLoadingId] = useState(null); // Track which banner is loading
  ...
  const pickBannerImage = async () => {
    showAlert(
      'Select Banner Image',
      'Choose an option to add a banner image:',
      [
          ...
          onPress: async () => {
            ...
            if (!result.canceled && result.assets && result.assets[0].uri) {
              const manipResult = await ImageManipulator.manipulateAsync(
                result.assets[0].uri,
                [{ resize: { width: 1280, height: 560 } }], // 16:7 ratio
  ...
  const uploadBanner = async () => {
    ...
    try {
      ...
      Alert.alert('Error uploading banner');
    }
    setUploading(false);
  };
  ...
  const removeBanner = async (id) => {
    ...
    try {
      ...
      Alert.alert('Error removing banner');
    }
    setLoadingId(null);
  };

  // Replace Alert.alert with this helper:
  function showAlert(title, message, actions) {
    if (Platform.OS === 'web') {
      if (!actions || actions.length === 1) {
        ...
        // Assume [Cancel, Confirm]
        if (window.confirm(message || title)) {
          if (actions[1].onPress) actions[1].onPress();
        }
      }
    } else {
